...
\cos\left(
\frac{1}{|A|}
\sum_{t \in A} \mathbf{v}(t),
\ \mathbf{v}(\mathrm{title}_i)
\right)
```
...
- 全体達成率
```math
\mathrm{overall} = \mathrm{mean}(\mathrm{achieved})
```
- カテゴリ別達成率
```math
\mathrm{cat\_rate}(c)
...
\mathrm{succ}_i
=
0.5 \cdot \mathrm{overall}
+
0.5 \cdot \mathrm{cat\_rate}(c_i)
```
...
-
(1-\lambda) \cdot
\max_{j \in S} \mathrm{pairwise}(i,j)
```
...
- 新規性：低類似度を優先
```math
\left( 1 - \mathrm{rem\_sim}_i^n \right)
```
- 未露出カテゴリを優先：履歴でのカテゴリ出現回数 `exposure` を min-max 正規化
```math
\left( 1 - \mathrm{exposure}_i^n \right)
```
- 難易度は3に近いほど良い（距離を負にして正規化）
```math
\mathrm{diff\_pref}_i
=
- \left| \mathrm{difficulty}_i - 3 \right|
```
- impact も加点
